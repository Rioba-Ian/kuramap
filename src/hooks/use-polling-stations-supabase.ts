 ...
 */
export function usePollingStations(options: UsePollingStationsOptions = {}): UsePollingStationsResult {
  const {
    userLocation,
    ...
    try {
      ...
      if (searchQuery && searchQuery.trim()) {
        ...
        fetchedStations = (data || []).map(station => ({
          id: station.id,
          name: station.name,
          address: station.address || `${station.ward} Ward, ${station.constituency}`,
      ...
      // Priority 2: Filter by constituency if provided
        ...
        const { data, error: constituencyError } = await supabase
          .rpc('get_stations_by_constituency', {
        ...
        if (ward && ward !== '') {
          fetchedStations = fetchedStations.filter(s =>
      ...
      // Priority 3: Get nearest stations if user location available
        ...
        const { data, error: nearestError } = await supabase
      ...
      // Fallback: Get first N stations (sorted by name)
        ...
        fetchedStations = (data || []).map(station => {
          ...
          const locationMatch = station.location?.toString().match(/POINT\(([-\d.]+)\s+([-\d.]+)\)/);
 ...
 */
export function getUniqueConstituencies(stations: RegistrationCenter[]): string[] {
 ...
 */
export function getUniqueWards(stations: RegistrationCenter[], constituency?: string): string[] {
  let filteredStations = stations;

  if (constituency && constituency.toLowerCase() !== 'all' && constituency !== '') {
